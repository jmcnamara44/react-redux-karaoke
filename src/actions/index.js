...
export function fetchLyrics(title, artist, musicMatchId, localSongId, dispatch) {
  return fetch('http://api.musixmatch.com/ws/1.1/track.lyrics.get?track_id=' + musicMatchId + '&apikey=').then(
    ...
    if (json.message.body.lyrics) {
      ...
      dispatch(changeSong(localSongId));
        ...
        console.log('we couldnt find the song');
    }
  });
}
  ...
  return function (dispatch) {
    ...
    return fetch('http://api.musixmatch.com/ws/1.1/track.search?&q_track=' + title + '&page_size=1&s_track_rating=desc&apikey=').then(
      ...
      if (json.message.body.track_list.length > 0) {
        ...
        console.log('We couldn\'t find a song under that ID!');
      }
    });
  };
}
